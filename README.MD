...
TODO (I'm back in 3 weeks): 
- night mode fix with `getWallpaperId()` in `editor.putString(LAST_WALLPAPER, WALLPAPER_NIGHT/WALLPAPER_NIGHT);` -> `editor.putInt(LAST_WALLPAPER, getWallpaperId());` to determine if the wallpaper has changed.
- improve the layouts and add 1920x1080 resolution.
- adding function to display fuel stats only in the selected apps leaving the main screen clear.
- separate canbus functions from Launcher.java, wrap them with try-catch block and create toast informing that modded by user class doesn't work as it should in case it doesn't. 
- update videos and screenshots for this readme.

So far the launcher has been tested on 2000x1100 and 1280x720 resolutions.
...
2. It should contain the name of the class that canbus runs as default, in my case it is : [`com.syu.carinfo.rzc.biaozhi408.RZC_BZ408IndexActi.java`](https://github.com/vasyl91/FYT-Launcher-Mod/blob/main/fuel_stats/RZC_BZ408IndexActi%20-%20ORIGINAL.java). **This is the file that sholud be modified!** Initially it contains stuff related to FYT canbus app that we don't want to run because it will crash the app so in order to run the fuel ststistics properly you will have to inspect some code and modify it as described below.
...
```
That originally leads to the class `com.syu.carinfo.rzc.biaozhi408.RZC_BZ408OilMileIndexActi.java` and you should keep looking there.
...
```
Copy and paste that section below and rename `android:name="com.syu.carinfo.rzc.biaozhi408.RZC_BZ408IndexActi"` accordingly to your newly modded file from step 7.
