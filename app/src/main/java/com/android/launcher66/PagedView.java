...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener,
    ...
    protected static boolean mIsSupportCircular; // = LauncherAppState.getInstance().getCircularSlidingScreenSwitch();
    private static final String TAG = "PagedView";
    private static final boolean DEBUG = false;
    protected static int INVALID_PAGE = mIsSupportCircular ? -999 : -1;

    // the min drag distance for a fling to register, to prevent random page shifts
    private static final int MIN_LENGTH_FOR_FLING = 25;

    protected static final int PAGE_SNAP_ANIMATION_DURATION = 600;
    ...
    protected static int INVALID_POINTER = mIsSupportCircular ? -999 : -1;
    ...
    private static final String KEY_DELAY_TIME = "persist.sys.la_delay_time";
    private static final String KEY_SLIP_VELOCITY = "persist.sys.la_slip_velocity";
    private static final String KEY_MIN_SNAP_VELOCITY = "persist.sys.la_min_snap_v";
    private static final String KEY_MIN_FLING_VELOCITY = "persist.sys.la_min_fling_v";
    private static final String KEY_TOUCH_SLOP = "persist.sys.la_touch_slop";
    private static final String KEY_FLING_THRESHOLD_VELOCITY = "persist.sys.la_fling_th_v";
    private static final int DEFAULT_TOUCH_SLOP_FOR_SLIDE_OPTIMIZATION = 8;
    private static final int DEFAULT_SLIDE_DAMPING_COEFFICIENT = 65;
    private float mDampingCoefficient = 6.5f;
    private int mPageSnapAnimDuration = 600;
    ...
    protected void init() {
        ...
        */
        ...
        mTouchSlop = systemPropertiesGetInt(KEY_TOUCH_SLOP, DEFAULT_TOUCH_SLOP_FOR_SLIDE_OPTIMIZATION);
        int tempFlingThresholdVelocity = systemPropertiesGetInt(KEY_FLING_THRESHOLD_VELOCITY, FLING_THRESHOLD_VELOCITY);
        int tempMinFlingVelocity = systemPropertiesGetInt(KEY_MIN_FLING_VELOCITY, MIN_FLING_VELOCITY);
        int tempMinSnapVelocity = systemPropertiesGetInt(KEY_MIN_SNAP_VELOCITY, MIN_SNAP_VELOCITY);
        mFlingThresholdVelocity = (int) (tempFlingThresholdVelocity * mDensity);
        mMinFlingVelocity = (int) (tempMinFlingVelocity * mDensity);
        mMinSnapVelocity = (int) (tempMinSnapVelocity * mDensity);
        mDampingCoefficient = (float) systemPropertiesGetInt(KEY_DELAY_TIME, DEFAULT_SLIDE_DAMPING_COEFFICIENT) / 10;
        mPageSnapAnimDuration = systemPropertiesGetInt(KEY_SLIP_VELOCITY, PAGE_SNAP_ANIMATION_DURATION);
     ...
     * Value changes to the next page's index when user starts to swipe.
     * @return The index of the currently displayed page.
     */
    public int getCurrentPage() {
        return mLogicalCurrentPage;
    }

    /**
     * Returns the index of the currently visible/moving page.
     * Value changes to the next page's index onPageEndMoving().
     * @return The index of the currently displayed page.
     */
    public int getCurrentlyUsedPage() {
        return mCurrentPage;
    }
    ...
    void setCurrentPage(int currentPage) {
        ...
        // UPDATE: Sync logical page
        mLogicalCurrentPage = mCurrentPage;
    ...
    public void scrollTo(int x, int y) {
        ...
        if (isXBeforeFirstPage) {
            super.scrollTo((mIsSupportCircular ? x : 0), y);
            ...
            super.scrollTo((mIsSupportCircular ? x : mMaxScrollX), y);
            if (mAllowOverScroll && !mIsSupportCircular) {
                if (isRtl) {
                    overScroll(x);
                } else {
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mIsSupportCircular && mNextPage == -1) {
                mCurrentPage = getPageCount() - 1;
                mLogicalCurrentPage = mCurrentPage;
                int x = (isLayoutRtl() ? 0 : mCurrentPage) * mViewport.width();
                scrollTo(x, getScrollY());
            } else if (mIsSupportCircular && mNextPage == getPageCount()) {
                mCurrentPage = 0;
                mLogicalCurrentPage = mCurrentPage;
                int x = (isLayoutRtl() ? (getPageCount() - 1) : mCurrentPage) * mViewport.width();
                scrollTo(x, getScrollY());
            } else {
                mCurrentPage = Math.max(0, Math.min(mNextPage, getPageCount() - 1));
                mLogicalCurrentPage = mCurrentPage;
            ...
            // Only notify if logical page wasn't already updated
            if (mPageSwitchListener != null) {
                mPageSwitchListener.onPageSwitch(getPageAt(mCurrentPage), mCurrentPage);
            }
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                Utilities.getDescendantCoordRelativeToParent(currPage, this, mTmpIntPoint, false);
                if (mTmpIntPoint[0] < 0 && (mIsSupportCircular ? (i < count -1) : true)) {
                    if (range[0] == -1) {
                        continue;
                    } else {
                        break;
            ...
            if (mIsSupportCircular && range[0] < 0) {
                range[0] = curScreen;
            }
            range[1] = curScreen;
        } else {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (pageCount > 0) {
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                ...
                if (mIsSupportCircular) {
                    dispatchDrawTempCanvas(leftScreen, rightScreen,canvas);
                }
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        if (xMoved || xPaged || yMoved) {
            if (mUsePagingTouchSlop ? xPaged : xMoved) {
                ...
                // UPDATE: Predict and set logical page based on scroll direction
                predictLogicalPageFromScroll(x);
            }
        }
    }

    protected void predictLogicalPageFromScroll(float currentX) {
        float deltaX = mDownMotionX - currentX;
        final boolean isRtl = isLayoutRtl();
        boolean isDeltaXLeft = isRtl ? deltaX < 0 : deltaX > 0;
        
        // Only predict if movement is significant enough
        if (Math.abs(deltaX) > mTouchSlop * 2) {
            if (isDeltaXLeft && mCurrentPage > (mIsSupportCircular ? -1 : 0)) {
                // Scrolling left (or right in RTL), predict previous page
                setLogicalCurrentPage(mCurrentPage - 1);
            } else if (!isDeltaXLeft && mCurrentPage < (mIsSupportCircular ? getChildCount() : getChildCount() - 1)) {
                // Scrolling right (or left in RTL), predict next page  
                setLogicalCurrentPage(mCurrentPage + 1);
            }
        }
    }
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        int totalDistance = mIsSupportCircular ? getViewportWidth() : (v.getMeasuredWidth() + mPageSpacing);
        int delta = screenCenter - (getScrollForPage(page) + halfScreenSize);
        if (mIsSupportCircular && !isLayoutRtl()) {
            if (mOverScrollX > mMaxScrollX && page == 0) {
                delta = screenCenter - mMaxScrollX - halfScreenSize - totalDistance;
            } else if (mOverScrollX < 0 && page == this.getChildCount() - 1) {
        ...
        if (mIsSupportCircular && isLayoutRtl()) {
            if (mOverScrollX < 0 && page == 0) {
                delta = screenCenter + halfScreenSize;
            } else if (mOverScrollX > mMaxScrollX && page == this.getChildCount() - 1) {
                delta = screenCenter - mMaxScrollX - halfScreenSize - totalDistance;
            }
        }
        float scrollProgress = delta / (totalDistance * 1.0f);
        scrollProgress = Math.min(scrollProgress, mIsSupportCircular ? 1.0f : getMaxScrollProgress());
        scrollProgress = Math.max(scrollProgress, mIsSupportCircular ? -1.0f : - getMaxScrollProgress());
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                            (isFling && !isVelocityXLeft)) && mCurrentPage > (mIsSupportCircular ? -1 : 0)) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                        
                        // UPDATE: Set logical page immediately
                        setLogicalCurrentPage(finalPage);
                        snapToPageWithVelocity(finalPage, velocityX);
                        
                    } else if (((isSignificantMove && isDeltaXLeft && !isFling) ||
                            (isFling && isVelocityXLeft)) &&
                            mCurrentPage < (mIsSupportCircular ? getChildCount() : getChildCount() - 1)) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1;
                    ...
                    if (nextPage != mCurrentPage) {
                        // UPDATE: Set logical page immediately
                        setLogicalCurrentPage(nextPage);
                        snapToPage(nextPage);
                    } else {
                        snapToDestination();
                    }
    ...
    protected void setLogicalCurrentPage(int page) {
        int oldLogicalPage = mLogicalCurrentPage;
        mLogicalCurrentPage = Math.max(0, Math.min(page, getPageCount() - 1));
        
        // Notify listeners immediately of logical page change
        if (oldLogicalPage != mLogicalCurrentPage && mPageSwitchListener != null) {
            mPageSwitchListener.onPageSwitch(getPageAt(mLogicalCurrentPage), mLogicalCurrentPage);
        }
        
        // Update page indicator immediately
        if (mPageIndicator != null && !isReordering(false)) {
            mPageIndicator.setActiveMarker(mLogicalCurrentPage);
        }
    }
    ...
    protected void snapToDestination() {
        snapToPage(getPageNearestToCenterOfScreen(), mPageSnapAnimDuration);
    }
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        whichPage = Math.max((mIsSupportCircular ? -1 : 0),
                Math.min(whichPage, (mIsSupportCircular ? getChildCount() : getChildCount()-1)));
        int halfScreenSize = getViewportWidth() / 2;
        ...
        if (Math.abs(velocity) < mMinFlingVelocity) {
            ...
            snapToPage(whichPage, mPageSnapAnimDuration);
            return;
        }
        ...
        duration = Math.round(mDampingCoefficient * 1000 * Math.abs(distance / velocity));
    ...
    protected void snapToPageImmediately(int whichPage) {
        snapToPage(whichPage, mPageSnapAnimDuration, true);
    }
    ...
    protected void snapToPage(int whichPage, int duration, boolean immediate) {
        whichPage = Math.max((mIsSupportCircular ? -1 : 0),
                (Math.min(whichPage, mIsSupportCircular ? getPageCount() : getPageCount() - 1)));
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate) {
        ...
        if (mIsSupportCircular && (whichPage == -1 || whichPage == getChildCount())) {
            if (isLayoutRtl()) {
                whichPage = (whichPage == -1) ? getChildCount() : -1;
            }
            final int newX = whichPage * mViewport.width();
    ...
    public void scrollLeft() {
        int targetPage = getNextPage() - 1;
        if (targetPage >= 0) {
            setLogicalCurrentPage(targetPage);
    ...
    private int systemPropertiesGetInt(String key, int def) {
        int value = def;
        try {
            if (mSystemProperties == null) {
                mSystemProperties = Class.forName(SYSTEM_PROPERTIES_CLASS_NAME);
            }
            if (mSystemPropertiesMethodGetInt == null) {
                mSystemPropertiesMethodGetInt = mSystemProperties.getMethod("getInt", String.class, int.class);
            }
            value = Integer.parseInt(mSystemPropertiesMethodGetInt.invoke(null, key, def).toString());
    ...
    private void dispatchDrawTempCanvas(int leftPage, int rightPage,Canvas canvas) {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        /*if (LauncherAppState.getCircularSlidingScreenSwitchKey().equals(key)) {
            mIsSupportCircular = LauncherAppState.getInstance().getCircularSlidingScreenSwitch();
            INVALID_PAGE = mIsSupportCircular ? -999 : -1;
            INVALID_POINTER = mIsSupportCircular ? -999 : -1;
        }*/
    }
